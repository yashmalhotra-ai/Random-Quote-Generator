...
function Body() {
  ...
  const handleBookmarkClick = (quote) => {
    const existingBookmarks = JSON.parse(localStorage.getItem('bookmarks')) || [];
    const isAlreadyBookmarked = existingBookmarks.some((bookmark) => bookmark._id === quote._id);
            ...
            randomQuote.map((item) => (
              <div key={item._id} className="flex flex-col mt-10">
                <div className="flex flex-col w-[500px] min-h-5 mb-2  bg-boxbackground rounded-[60px]">
                  ...
                  <div onClick={() => handleBookmarkClick(item)} className=" relative left-[30%] top-[-15px] item-end m-auto leading-relaxed text-base hover:cursor-pointer">
